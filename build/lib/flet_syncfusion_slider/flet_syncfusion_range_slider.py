...
class SyncfusionRangeSlider(ConstrainedControl, AdaptiveControl):
    """
    A Syncfusion range slider provides advanced range selection with customizable appearance,
    interactive elements like ticks and labels, and extensive event handling.

    Use this range slider when you need to select a range of values with advanced functionalities,
    ...
    def main(page):
        def range_slider_changed(e):
            print(f"Range slider changed to {e.control.start_value} - {e.control.end_value}")

        page.add(
            ft.Text("Syncfusion Range Slider with values:"),
            ft.SyncfusionRangeSlider(
                start_value=20,
                end_value=80,
                min=0,
                max=100,
    ...
    def __init__(
        self,
        start_value: OptionalNumber = None,
        end_value: OptionalNumber = None,
    ...
    def _get_control_name(self):
        return "flet_syncfusion_range_slider"
    ...
    def before_update(self):
        ...
        assert (
            self.min is None
            or self.start_value is None
            or (self.start_value >= self.min)
        ), "start_value must be greater than or equal to min"
        assert (
            self.max is None or self.end_value is None or (self.end_value <= self.max)
        ), "end_value must be less than or equal to max"
        assert (
            self.start_value is None
            or self.end_value is None
            or (self.start_value <= self.end_value)
        ), "start_value must be less than or equal to end_value"

        # Sincronizar start_value y end_value con start y end
        if (
            self.start_value is not None
            and self.min is not None
            and self.max is not None
        ):
            start = max(min(self.start_value, self.max), self.min)
            self._set_attr("start", start)

        if self.end_value is not None and self.min is not None and self.max is not None:
            end = max(min(self.end_value, self.max), self.min)
            self._set_attr("end", end)
    ...
    def _handle_event(self, e):
        if e.name == "change" and self.on_change is not None:
            values = e.data.split(",")
            if len(values) == 2:
                start_val = float(values[0])
                end_val = float(values[1])
                self._set_attr("start", start_val)
                self._set_attr("end", end_val)
                # Actualizar también start_value y end_value para mantener sincronización
                self._set_attr("start_value", start_val)
                self._set_attr("end_value", end_val)
                self.on_change(e)
    ...
    def start_value(self) -> float:
        return self._get_attr(
            "start_value", data_type="float", def_value=self.min or 0.0
        )

    @start_value.setter
    def start_value(self, value: OptionalNumber):
        self._set_attr("start_value", value)
        # Cuando se actualiza start_value, también actualizar start si min y max están definidos
        if value is not None and self.min is not None and self.max is not None:
            start = max(min(value, self.max), self.min)
            self._set_attr("start", start)
    ...
    def end_value(self) -> float:
        return self._get_attr(
            "end_value", data_type="float", def_value=self.max or 100.0
        )

    @end_value.setter
    def end_value(self, value: OptionalNumber):
        self._set_attr("end_value", value)
        # Cuando se actualiza end_value, también actualizar end si min y max están definidos
        if value is not None and self.min is not None and self.max is not None:
            end = max(min(value, self.max), self.min)
            self._set_attr("end", end)
    ...
    def _get_children(self):
        ...
        if hasattr(self, "start_thumb_icon") and self.start_thumb_icon is not None:
            self.start_thumb_icon._set_attr_internal("n", "start_thumb_icon")
            children.append(self.start_thumb_icon)
        if hasattr(self, "end_thumb_icon") and self.end_thumb_icon is not None:
            self.end_thumb_icon._set_attr_internal("n", "end_thumb_icon")
            children.append(self.end_thumb_icon)
        return children
...
class SyncfusionVerticalRangeSlider(SyncfusionRangeSlider):
    """
    A vertical version of the Syncfusion range slider with all the same features but in vertical orientation.
    ...
    def main(page):
        def range_slider_changed(e):
            print(f"Vertical range slider changed to {e.control.start_value} - {e.control.end_value}")

        page.add(
            ft.Text("Syncfusion Vertical Range Slider with values:"),
            ft.SyncfusionVerticalRangeSlider(
                start_value=20,
                end_value=80,
                min=0,
                max=100,
